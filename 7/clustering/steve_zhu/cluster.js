...
var assign = function(centroids, data) {
	...
	items.attr('stroke-width', 3)
		.attr('stroke', function(d) {
			return clustercolors[d.type];
		})
		.attr('fill', function(d) {
			return clustercolors[d.realtype];
		});
}
...
var doit = function(d) {
	...
	data = _.map(d, function(d) {
		return {
			'realtype': parseInt(d.class),
			'type': parseInt(d.class),
			features: [parseFloat(d.participants),
				parseFloat(d.female),
				parseFloat(d.male),
				parseFloat(d.us_citizen)
			]
		};
	});
	...
	items = svg.selectAll("item")
		...
		.attr('r', 5)
		.attr('cx', function(d) {
			return xScale(d.features[0]);
		})
		.attr('cy', function(d) {
			return yScale(d.features[1]);
		})
		.attr('stroke-width', 3)
		.attr('fill', function(d) {
			return clustercolors[d.realtype];
...
var dist = function(a, b) {
	return _.reduce(_.map(_.zip(a, b), function(d) {
